...
final class LayoutBarItemView: NSView {
    ...
    func provideAlertForDisabledItem() -> NSAlert {
        ...
        alert.informativeText = String(
            format: String(localized: "macOS prohibits \"%@\" from being moved."),
            item.displayName
        )
        return alert
    }
    ...
    func provideAlertForUnresponsiveItem() -> NSAlert {
        ...
        alert.informativeText = String(
            format: String(localized: "%@ is unresponsive. Until it is restarted, it cannot be moved. Movement of other menu bar items may also be affected until this is resolved."),
            item.displayName
        )
        return alert
    }
